...
export const getRequestsByEmail = (email) => {
    return axios.get(`http://localhost:8080/api/v1/requests/by-email?email=${encodeURIComponent(email)}`);
}

export const rejectRequest = (requestId, reason) => {
    return axios.post(`http://localhost:8080/api/v1/recipient/${requestId}/reject`, reason, {
        headers: {
