...
export function* searchDonor(request) {
    try {
        let data;
        // Support all types of search including fetching all organ donors
        if (request.searchType === 'all') {
            // Fetch all active organ donors
            const response = yield call(findAll, 'ACTIVE');
            // Filter to only include donors with organ data
            data = response.data.filter(donor => donor.organType !== null);
        } else {
            // Support both blood and organ search
            const response = yield call(search, request.pincode, request.bloodGroup, request.searchType);
            data = response.data;
        }
        ...
        console.error("Error comes while searching organ/blood donor : " + message);
    }
}
