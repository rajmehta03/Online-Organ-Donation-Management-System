...
class NeedOrgan extends React.Component {
    ...
    handleSubmit = (event) => {
        ...
        // Check if hospital is logged in and add requesterHospitalId
        const hospital = localStorage.getItem('hospital');
        let payloadToSend = { ...this.state.payload };
        
        if (hospital) {
            const hospitalData = JSON.parse(hospital);
            payloadToSend.requesterHospitalId = hospitalData.id;
        }
        
        this.props.register(this.state.prescription, payloadToSend);
        setTimeout(() => {
            this.setState({
                ...this.state,
                payload: {
    ...
    render() {
        ...
        const { patientName,
            ...
            reason,
            requiredOrganType,
            organQuantityRequired } = this.state.payload;
                ...
                <Row>
                    <Col>
                        <h2 className={classes.pageHeading}>
                            <FontAwesomeIcon icon="bed" /> Need Organ To Save Life
                                </h2>
                        <hr />
                    </Col>
                </Row>
                ...
                <Row>
                    <Col>
                        <h3 className={classes.formHeading}>
                            <FontAwesomeIcon icon="bed" /> Request Organ to Save Life
                            </h3>
                    </Col>
                </Row>
                <Row>
                    <Col>
                        <Form className={classes.form} onSubmit={this.handleSubmit}>
                            ...
                            <Row form>
                                <Col md={6}>
                                    <FormGroup>
                                        <Label for="organType" className={classes.Label}>Required Organ Type</Label>
                                        <Input type="select" name="requiredOrganType" id="organType" onChange={this.changeHandler} value={requiredOrganType}>
                                ...
                                <Col md={6}>
                                    <FormGroup>
                                        <Label for="organQuantity" className={classes.Label}>Number of Organs Required</Label>
                                        <Input type="number"
                                            id="organQuantity"
                                            name="organQuantityRequired"
                                            min="1"
                                            placeholder="Enter quantity"
                                            onChange={this.changeHandler}
                                            value={organQuantityRequired} />
                                    </FormGroup>
                                </Col>
                            </Row>
                            <Row form>
                            ...
                            <Row form>
                                <Col md={6}>
                                    <FormGroup>
                                        <Label for="organReason" className={classes.Label}>Reason For Organ</Label>
                                        <Input type="textarea"
                                            id="organReason"
                                            name="reason"
...
export default connect(mapStateToProps, mapDispatchToProps)(NeedOrgan);
