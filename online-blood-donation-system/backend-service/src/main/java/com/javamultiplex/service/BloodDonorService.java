...
public class BloodDonorService {
    ...
    public BloodDonor register(String request, MultipartFile profileImage) {
        ...
        try {
            ...
            System.out.println("Parsed DTO: " + bloodDonorDTO);
    ...
    /**
     * Search donors by organ type
     * @param zip
     * @param organType
     * @return
     */
    public List<BloodDonor> searchByOrgan(String zip, String organType) {
        try {
            organType = URLDecoder.decode(organType, StandardCharsets.UTF_8.name());
        } catch (UnsupportedEncodingException e) {
            throw new ServiceException(ErrorResponseDTO
                    .builder()
                    .statusCode(500)
                    .developerMessage(ExceptionUtils.getRootCauseMessage(e))
                    .userMessage("Exception comes while decoding request parameter (organ type)")
                    .build());
        }
        OrganType organ = OrganType.valueOf(organType);
        return bloodDonorRepository.findAllByAddressZipAndOrganTypeAndOrganActiveAndStatus(
                zip, organ, true, BloodDonorStatus.ACTIVE);
    }
     ...
     * @return
     */
    public List<BloodDonor> getAllOrganDonors() {
        return bloodDonorRepository.findAllByOrganTypeIsNotNullAndOrganActiveAndStatus(true, BloodDonorStatus.ACTIVE);
    }
     ...
     * @return
     */
    public BloodDonor save(BloodDonor bloodDonor) {
        return bloodDonorRepository.save(bloodDonor);
    }
}